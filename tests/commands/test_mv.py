...
def test_resolve_destination_path_to_existing_directory():
    ...
    with mock.patch("pathlib.Path.exists", return_value=True), mock.patch("pathlib.Path.is_dir", return_value=True):
...
def test_resolve_destination_path_to_existing_file():
    ...
    with mock.patch("pathlib.Path.exists", return_value=True), mock.patch("pathlib.Path.is_dir", return_value=False):
        result = _resolve_destination_path(source, destination)
        assert result == destination
...
def test_update_metadata_file_path_missing_distribution():
    ...
    with mock.patch("biotope.commands.mv.Path") as mock_path:
        ...
        metadata = {"@context": {"@vocab": "https://schema.org/"}, "@type": "Dataset", "name": "test"}

        with mock.patch("builtins.open", mock.mock_open(read_data=json.dumps(metadata))):
            result = _update_metadata_file_path(
                Path("/fake/metadata.jsonld"), "old/path.csv", "new/path.csv", "checksum", Path("/fake/root")
            )
            assert result is False
...
def test_update_metadata_file_path_multiple_distributions():
    ...
    metadata = {
        ...
        "name": "test",
        "distribution": [
            {"@type": "cr:FileObject", "contentUrl": "other/file.csv", "sha256": "other_checksum"},
            {"@type": "cr:FileObject", "contentUrl": "old/path.csv", "sha256": "old_checksum"},
        ],
    }
    ...
    with mock.patch("builtins.open", mock_file), mock.patch("pathlib.Path.stat", return_value=mock_stat):
        result = _update_metadata_file_path(
            Path("/fake/metadata.jsonld"), "old/path.csv", "new/path.csv", "new_checksum", biotope_root
        )
...
def test_update_metadata_file_path_file_size_error():
    ...
    with mock.patch.object(Path, "__truediv__") as mock_div:
        ...
        metadata = {
            ...
            "@type": "Dataset",
            "name": "test",
            "distribution": [{"@type": "cr:FileObject", "contentUrl": "old/path.csv", "sha256": "old_checksum"}],
        }

        with mock.patch("builtins.open", mock.mock_open(read_data=json.dumps(metadata))):
            result = _update_metadata_file_path(
                Path("/fake/metadata.jsonld"), "old/path.csv", "new/path.csv", "new_checksum", Path("/fake/root")
            )
...
def test_find_metadata_files_corrupted_json():
    ...
    with mock.patch.object(Path, "exists", return_value=True), mock.patch.object(Path, "rglob") as mock_rglob:
        # Create mock files
        ...
        # Mock file content
        good_metadata = {"distribution": [{"@type": "cr:FileObject", "contentUrl": "test/file.csv"}]}
        ...
        with mock.patch("biotope.commands.mv.is_git_repo", return_value=True):
            with mock.patch("biotope.commands.mv.is_file_tracked", return_value=True):
                with mock.patch("biotope.commands.mv.stage_git_changes") as mock_stage:
                    ...
                    metadata1 = (
                        biotope_project_with_directory / ".biotope" / "datasets" / "moved_experiment" / "data1.jsonld"
                    )
                    metadata2 = (
                        biotope_project_with_directory
                        / ".biotope"
                        / "datasets"
                        / "moved_experiment"
                        / "subdir"
