...
def sample_remote_config():
    ...
    return {
        "annotation_validation": {
            "enabled": True,
            "minimum_required_fields": ["name", "description", "creator", "dateCreated", "distribution", "license"],
            "field_validation": {
...
def test_set_remote_validation(mock_find_root, runner, biotope_project):
    ...
    initial_config = {"annotation_validation": {"enabled": True, "minimum_required_fields": ["name", "description"]}}
    ...
    with mock.patch("biotope.validation._load_remote_validation_config") as mock_load_remote:
        mock_load_remote.return_value = {
            ...
            "field_validation": {"creator": {"type": "object", "required_keys": ["name"]}},
        }
