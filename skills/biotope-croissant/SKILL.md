...
**The unit of work.** One **capability**: a question family the graph is claimed to answer, the interpretations needed to read it correctly, a validation check comparing it with source evidence, and one runnable query example. A capability with no check reports `unchecked`.

**Ownership.** The user owns scientific decisions. You own engineering ones. When defensible readings require different populations, explain the tradeoff and agree which readings the graph should support.
...
**Scientific choices.** Reuse decisions already supplied by the user. Ask about missing choices that affect selection or identity. Explain limitations when available evidence cannot support a requested claim.
...
- [ ] Step 7: Review capabilities and hand over
```
...
```bash
rg -n -g '*.py' '\bcontinue\b|^\s*pass$' graph/pipelines graph/mappings
```
...
**Step 7: Review capabilities and hand over.** Run declared query examples when querying is part of the authorized task and an appropriate backend is available. Otherwise report them as unexecuted. Then report:
...
- [interpretation.md](./references/interpretation.md) — steps 2 and 7: what a capability declares, and what counts as evidence supporting it.
