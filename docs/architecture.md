...
│   ├── acquisition/       DuckDB row streaming + minimal transforms
│   ├── mapping/           mapping.yaml schema + compiler
│   ├── alignment/         alignment.yaml schema + cross-Croissant merge
...
raw files ──► biotope add ──► .biotope/datasets/<name>.jsonld    (Croissant; baker-enriched)
                              │
                              ├─► biotope propose-mapping ──► mappings/<name>.mapping.yaml
                              │
                              ├─► biotope propose-alignment ──► alignment.yaml      (optional, multi-dataset)
                              │
                              ▼
                       biotope build ──► build/                  (schema_config.yaml + adapter)
                                          │
                                          ▼
...
Each transformation is deterministic. The agent (or user) only authors `.biotope/project.yaml` (purpose) and may refine the proposed `mapping.yaml` / `alignment.yaml` before `build`.
...
| `mappings/*.mapping.yaml`     | proposed       | Per-dataset RecordSet → BioCypher node/edge mapping  |
| `alignment.yaml`              | proposed       | Cross-dataset same_node equivalences                 |
