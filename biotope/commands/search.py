...
@click.option(
    "--sort",
    "-s",
    type=click.Choice(["relevance", "impact", "name", "composite"]),
    default="relevance",
    help="Sort results by relevance, impact, name, or composite score",
)
def search(query: Optional[str], limit: int, type: Optional[str], sort: str) -> None:
    ...
    if search_type == "mcp":
        ...
        if sort == "relevance":
            ...
            all_results.sort(key=lambda x: (-get_impact_value(x), x.get("name", "").lower()))
        elif sort == "name":
