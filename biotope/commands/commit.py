...
def _stage_biotope_changes(biotope_root: Path) -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "status", "--porcelain", ".biotope/"], cwd=biotope_root, capture_output=True, text=True, check=True
        )
...
def _create_git_commit(biotope_root: Path, message: str, author: Optional[str], amend: bool) -> Optional[str]:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "rev-parse", "HEAD"], cwd=biotope_root, capture_output=True, text=True, check=True
        )
